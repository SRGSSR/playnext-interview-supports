/**
 *
 * Please note:
 * This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * Do not edit this file manually.
 *
 */

@file:Suppress(
    "ArrayInDataClass",
    "EnumEntryName",
    "RemoveRedundantQualifierName",
    "UnusedImport"
)

package ch.srg.sam.generated.model

import io.kotlintest.shouldBe
import io.kotlintest.specs.ShouldSpec

import ch.srg.sam.generated.model.TopicDataDto
import ch.srg.sam.generated.model.TransmissionDto

class TopicDataDtoTest : ShouldSpec() {
    init {
        // uncomment below to create an instance of TopicDataDto
        //val modelInstance = TopicDataDto()

        // to test the property `externalId` - Unique identifier as defined by the business unit
        should("test externalId") {
            // uncomment below to test the property
            //modelInstance.externalId shouldBe ("TODO")
        }

        // to test the property `transmission` - Primary content transmission channel
        should("test transmission") {
            // uncomment below to test the property
            //modelInstance.transmission shouldBe ("TODO")
        }

        // to test the property `title`
        should("test title") {
            // uncomment below to test the property
            //modelInstance.title shouldBe ("TODO")
        }

        // to test the property `description` - Description of the topic
        should("test description") {
            // uncomment below to test the property
            //modelInstance.description shouldBe ("TODO")
        }

        // to test the property `imageCopyright`
        should("test imageCopyright") {
            // uncomment below to test the property
            //modelInstance.imageCopyright shouldBe ("TODO")
        }

        // to test the property `imageTitle`
        should("test imageTitle") {
            // uncomment below to test the property
            //modelInstance.imageTitle shouldBe ("TODO")
        }

        // to test the property `imageUrl`
        should("test imageUrl") {
            // uncomment below to test the property
            //modelInstance.imageUrl shouldBe ("TODO")
        }

        // to test the property `lead` - Short description of the topic
        should("test lead") {
            // uncomment below to test the property
            //modelInstance.lead shouldBe ("TODO")
        }

        // to test the property `sortPriority` - Sort priority of topic, no rules, duplicates and gaps are allowed.In public API lower values will be returned first, null values last.
        should("test sortPriority") {
            // uncomment below to test the property
            //modelInstance.sortPriority shouldBe ("TODO")
        }

    }
}
